...
public class OrderViewHolder<MESSAGE extends IMessage> extends BaseMessageViewHolder<MESSAGE>
    ...
    public OrderViewHolder(View itemView, boolean isSender) {
        ...
        mProductImg = (ImageView ) itemView.findViewById(R.id.aurora_custom_order_img);
        mProductNameTv = (TextView) itemView.findViewById(R.id.aurora_custom_order_name);
        mProductPriceTv = (TextView ) itemView.findViewById(R.id.aurora_custom_order_price);

        mAddressNameTv =(TextView) itemView.findViewById(R.id.aurora_custom_order_address_name);
        mAddressPhoneTv =(TextView) itemView.findViewById(R.id.aurora_custom_order_address_phone);
        mAddressAreaTv =(TextView) itemView.findViewById(R.id.aurora_custom_order_address_area);

        mOrderIdTv = (TextView ) itemView.findViewById(R.id.aurora_custom_order_id);
        mOrderStatusTv = (TextView ) itemView.findViewById(R.id.aurora_custom_order_status);
    }
...
//        mImageLoader.loadImage(mProductImg, messageExtra.get("orderImg"));
        ...
        mAddressNameTv.setText("姓名：" + messageExtra.get("orderConsigneeName"));
        mAddressPhoneTv.setText("电话：" + messageExtra.get("orderConsigneePhone"));
        mAddressAreaTv.setText("地址：" + messageExtra.get("orderConsigneeAddress"));

        mOrderIdTv.setText("订单编号：" + messageExtra.get("orderId"));
        mOrderStatusTv.setText(messageExtra.get("orderStatus"));
        ...
        if (mIsSender) {
            switch (message.getMessageStatus()) {
                case SEND_GOING:
                    ...
                    Log.i("ProductViewHolder", "sending message");
                    break;
                case SEND_FAILED:
                    mSendingPb.setVisibility(View.GONE);
                    Log.i("ProductViewHolder", "send message failed");
                ...
                case SEND_SUCCEED:
                    mSendingPb.setVisibility(View.GONE);
                    mResendIb.setVisibility(View.GONE);
                    Log.i("ProductViewHolder", "send message succeed");
                    break;
            }
        }
